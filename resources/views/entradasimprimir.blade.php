...
<div class="container mt-4" style="font-size: 14px;">
    ...
    {{-- CHECKLIST --}}
@if($checklist)
    <h5 class="mt-4">CHECK LIST DE LA UNIDAD 
    ...
    <table class="table table-bordered table-checklist text-center">
        ...
        <tbody>
            <tr>
                ...
                <td>{{ isset($checklist->documentos_completos) && $checklist->documentos_completos ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>Accesorios Completos</td>
                <td>{{ isset($checklist->accesorios_completos) && $checklist->accesorios_completos ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
            ...
            <tr>
                <td>PDI Realizada</td>
                <td>{{ isset($checklist->pdi_realizada) && $checklist->pdi_realizada ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>Seguro Vigente</td>
                <td>{{ isset($checklist->seguro_vigente) && $checklist->seguro_vigente ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>NFC Instalado</td>
                <td>{{ isset($checklist->nfc_instalado) && $checklist->nfc_instalado ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>GPS Instalado</td>
                <td>{{ isset($checklist->gps_instalado) && $checklist->gps_instalado ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>Folder Viajero</td>
                <td>{{ isset($checklist->folder_viajero) && $checklist->folder_viajero ? '✔️' : '❌' }}</td>
                <td></td>
            </tr>
            <tr>
                <td>Recibido Por</td>
...
@else
    <div class="alert alert-warning mt-4">
        No se encontró checklist de entrada ni de salida para esta orden.
    </div>
@endif
    ...
    <div class="d-print-none mt-4 text-center">
        ...
        {{-- <a href="{{ route('admin.vehiculos') }}" class="btn btn-secondary mx-2">Regresar</a> --}}
        <a href="{{ route('admin.vehiculos') }}" target="_self" class="btn btn-secondary mx-2">Regresar</a>
...
/* Aquí puedes poner CSS específico para impresión */
</style>
@stop
