...
//         $query = Entrada::with(['almacenEntrada', 'almacenSalida', 'vehiculo'])
//             ->orderBy('Fecha_entrada', 'desc');

//         if (!Auth::user()->isAdmin()) {
//             $query->where('Almacen_entrada', Auth::user()->almacen_id);
//         }
...
//             'Tipo' => 'required|in:Madrina,Traspaso,Devolucion',
...
//         return redirect()->route('admin.entradas')->with('success', 'Entrada confirmada y vehículo en almacén.');
//     }
...
//         // ✅ Cancelar salida vinculada
//         $salida = Salida::where('VIN', $entrada->VIN)
//                         ->where('estatus', 'pendiente')
//                         ->latest('Fecha')
//                         ->first();
//         if ($salida) {
//             $salida->estatus = 'cancelada';
//             $salida->save();
//         }

//         // El vehículo sigue en el mismo almacén
//         $vehiculo->estatus = 'En almacén';
//         $vehiculo->save();

//         return redirect()->route('admin.entradas')->with('success', 'Entrada rechazada correctamente.');
//     }
// }
    ...
    public function index()
        {
            ...
            if ($user->role === 'admin') {
                // Admin ve todas
                $entradas = Entrada::with('almacen')
                                ->orderBy('Fecha_entrada', 'desc')
                                ->paginate(10);
            } else {
                // Usuario ve solo las entradas de su almacén
                $entradas = Entrada::with('almacen')
                                ->where('Almacen_entrada', $user->almacen_id)
                                ->orderBy('Fecha_entrada', 'desc')
                                ->paginate(10);
            }
    ...
    public function importar(Request $request)
{
    ...
    try {
        ...
        return back()->withErrors([
            'error' => 'Importación cancelada: ' . $e->getMessage()
        ]);
    }
}
