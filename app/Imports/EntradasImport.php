    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($validator->fails()) {
              ...
              // --- Validación VIN duplicado en la BD ---
                $vin = $row['vin'] ?? null;
                if (Vehiculo::where('VIN', $vin)->exists()) {
                    throw new \Exception("El VIN {$vin} ya existe en la base de datos. No se pudo importar el archivo.");
                }
